...
{
  // TODO: I think this is a clone of test_q_rhashmap_5.c
  // Once that is verified, this can be deleted
  int status = 0;
  KEYTYPE *keys = NULL;
  VALTYPE *vals = NULL;
  ...
  //------------------------------------------------
  t_stop = RDTSC();
  fprintf(stdout, "Passsed  %s cycles = %" PRIu64 "\n", __func__, (t_stop-t_start));
BYE:
